...
echo '<h1 class="page-header"><i class="bi bi-gear-fill me-2"></i>Website Settings</h1>';
...
if(isset($_POST['settings_submit'])) {
	try {
		...
		// website status
		if(!isset($_POST['system_active'])) {
			throw new RuntimeException('Invalid Website Status setting.');
		}
		if(!in_array($_POST['system_active'], array('0', '1', 0, 1))) {
			throw new RuntimeException('Invalid Website Status setting.');
		}
		$setting['system_active'] = $_POST['system_active'] == 1;
		
		// error reporting
		if(!isset($_POST['error_reporting'])) {
			throw new RuntimeException('Invalid Error Reporting setting.');
		}
		if(!in_array($_POST['error_reporting'], array('0', '1', 0, 1))) {
			throw new RuntimeException('Invalid Error Reporting setting.');
		}
		...
		if(!isset($_POST['maintenance_page'])) {
			throw new RuntimeException('Invalid Maintenance Page setting.');
		}
		if(!Validator::Url($_POST['maintenance_page'])) {
			throw new RuntimeException(
				'The maintenance page setting is not a valid URL.'
			);
		}
		...
		if(!isset($_POST['server_name'])) {
			throw new RuntimeException('Invalid Server Name setting.');
		}
		$setting['server_name'] = $_POST['server_name'];
		
		// website title
		if(!isset($_POST['website_title'])) {
			throw new RuntimeException('Invalid Website Title setting.');
		}
		$setting['website_title'] = $_POST['website_title'];
		
		// meta description
		if(!isset($_POST['website_meta_description'])) {
			throw new RuntimeException('Invalid Meta Description setting.');
		}
		$setting['website_meta_description'] = $_POST['website_meta_description'];
		
		// meta keywords
		if(!isset($_POST['website_meta_keywords'])) {
			throw new RuntimeException('Invalid Meta Keywords setting.');
		}
		$setting['website_meta_keywords'] = $_POST['website_meta_keywords'];
		
		// forum link
		if(!isset($_POST['website_forum_link'])) {
			throw new RuntimeException('Invalid Forum Link setting.');
		}
		if(!Validator::Url($_POST['website_forum_link'])) {
			throw new RuntimeException('The forum link setting is not a valid URL.');
		}
		$setting['website_forum_link'] = $_POST['website_forum_link'];
		
		// language switch
		if(!isset($_POST['language_switch_active'])) {
			throw new RuntimeException('Invalid Language Switch setting.');
		}
		if(!in_array($_POST['language_switch_active'], array('0', '1', 0, 1))) {
			throw new RuntimeException('Invalid Language Switch setting.');
		}
		$setting['language_switch_active'] = ($_POST['language_switch_active']
			== 1);
		...
		// language debug
		if(!isset($_POST['language_debug'])) {
			throw new RuntimeException('Invalid Language Debug setting.');
		}
		if(!in_array($_POST['language_debug'], array('0', '1', 0, 1))) {
			throw new RuntimeException('Invalid Language Debug setting.');
		}
		$setting['language_debug'] = ($_POST['language_debug'] == 1);
		
		// plugin system
		if(!isset($_POST['plugins_system_enable'])) {
			throw new RuntimeException('Invalid Plugin System setting.');
		}
		if(!in_array($_POST['plugins_system_enable'], array('0', '1', 0, 1))) {
			throw new RuntimeException('Invalid Plugin System setting.');
		}
		$setting['plugins_system_enable'] = ($_POST['plugins_system_enable']
			== 1);
		
		// ip block system
		if(!isset($_POST['ip_block_system_enable'])) {
			throw new RuntimeException('Invalid IP Block System setting.');
		}
		if(!in_array($_POST['ip_block_system_enable'], array('0', '1', 0, 1))) {
			throw new RuntimeException('Invalid IP Block System setting.');
		}
		$setting['ip_block_system_enable'] = ($_POST['ip_block_system_enable']
			== 1);
		
		// player_profiles
		if(!isset($_POST['player_profiles'])) {
			throw new RuntimeException('Invalid setting (player_profiles)');
		}
		if(!in_array($_POST['player_profiles'], array('0', '1', 0, 1))) {
			throw new RuntimeException('Invalid setting (player_profiles)');
		}
		$setting['player_profiles'] = ($_POST['player_profiles'] == 1);
		
		// guild_profiles
		if(!isset($_POST['guild_profiles'])) {
			throw new RuntimeException('Invalid setting (guild_profiles)');
		}
		if(!in_array($_POST['guild_profiles'], array('0', '1', 0, 1))) {
			throw new RuntimeException('Invalid setting (guild_profiles)');
		}
		$setting['guild_profiles'] = ($_POST['guild_profiles'] == 1);
		
		// username_min_len
		if(!isset($_POST['username_min_len'])) {
			throw new RuntimeException('Invalid setting (username_min_len)');
		}
		if(!Validator::UnsignedNumber($_POST['username_min_len'])) {
			throw new RuntimeException('Invalid setting (username_min_len)');
		}
		$setting['username_min_len'] = $_POST['username_min_len'];
		
		// username_max_len
		if(!isset($_POST['username_max_len'])) {
			throw new RuntimeException('Invalid setting (username_max_len)');
		}
		if(!Validator::UnsignedNumber($_POST['username_max_len'])) {
			throw new RuntimeException('Invalid setting (username_max_len)');
		}
		$setting['username_max_len'] = $_POST['username_max_len'];
		
		// password_min_len
		if(!isset($_POST['password_min_len'])) {
			throw new RuntimeException('Invalid setting (password_min_len)');
		}
		if(!Validator::UnsignedNumber($_POST['password_min_len'])) {
			throw new RuntimeException('Invalid setting (password_min_len)');
		}
		$setting['password_min_len'] = $_POST['password_min_len'];
		
		// password_max_len
		if(!isset($_POST['password_max_len'])) {
			throw new RuntimeException('Invalid setting (password_max_len)');
		}
		if(!Validator::UnsignedNumber($_POST['password_max_len'])) {
			throw new RuntimeException('Invalid setting (password_max_len)');
		}
		$setting['password_max_len'] = $_POST['password_max_len'];
		
		// cron_api
		if(!isset($_POST['cron_api'])) {
			throw new RuntimeException('Invalid setting (cron_api)');
		}
		if(!in_array($_POST['cron_api'], array('0', '1', 0, 1))) {
			throw new RuntimeException('Invalid setting (cron_api)');
		}
		$setting['cron_api'] = ($_POST['cron_api'] == 1);
		...
		if(isset($_POST['social_link_facebook'])
			&& !Validator::Url(
				$_POST['social_link_facebook']
			)
		) {
			throw new RuntimeException(
				'The facebook link setting is not a valid URL.'
			);
		}
		$setting['social_link_facebook'] = $_POST['social_link_facebook'];
		
		// social link instagram
		if(isset($_POST['social_link_instagram'])
			&& !Validator::Url(
				$_POST['social_link_instagram']
			)
		) {
			throw new RuntimeException(
				'The instagram link setting is not a valid URL.'
			);
		}
		$setting['social_link_instagram'] = $_POST['social_link_instagram'];
		
		// social link discord
		if(isset($_POST['social_link_discord'])
			&& !Validator::Url(
				$_POST['social_link_discord']
			)
		) {
			throw new RuntimeException(
				'The discord link setting is not a valid URL.'
			);
		}
		...
		// maximum online
		if(isset($_POST['maximum_online'])
			&& !Validator::UnsignedNumber(
				$_POST['maximum_online']
			)
		) {
...
echo '<div class="acp-card">';
	echo '<div class="acp-card-header">Configuration</div>';
		...
		echo '<table class="table table-hover module_config_tables" style="table-layout:fixed;">';
			...
			echo '<tr>';
				echo '<td>';
					...
					echo '<p class="setting-description">Default language that Darkheim will use.</p>';
				echo '</td>';
				echo '<td>';
			...
			echo '<tr>';
				echo '<td>';
					...
					echo '<p class="setting-description">Enables/disabled language debugging. If enabled, language phrases will be shown in a hover-tip when pointing text with the mouse. Keep disabled in a live website.</p>';
				echo '</td>';
				echo '<td>';
					echo '<div class="radio">';
						echo '<label>';
			...
			echo '<tr>';
				echo '<td>';
					...
					echo '<p class="setting-description">Maximum number of players that your server may allow. Leave empty to hide this information.</p>';
				echo '</td>';
				echo '<td>';
