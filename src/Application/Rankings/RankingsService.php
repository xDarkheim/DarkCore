...
class RankingsService
{
    ...
    {
        switch ($type) {
            case 'level':        $this->_levelsRanking();
                break;
            case 'resets':       $this->_resetsRanking();
                break;
            case 'killers':      $this->_killersRanking();
                break;
            case 'grandresets':  $this->_grandresetsRanking();
                break;
            case 'online':       $this->_onlineRanking();
                break;
            case 'votes':        $this->_votesRanking();
                break;
            case 'guilds':       $this->_guildsRanking();
                break;
            case 'master':       $this->_masterlevelRanking();
                break;
            case 'gens':         $this->_gensRanking();
                break;
        }
    }
    ...
    private function _votesRanking(): void
    {
        ...
        foreach ($accounts as $data) {
            ...
            if (in_array($characterName, $this->_excludedCharacters, true)) {
                continue;
            }
    ...
    {
        $customConfig   = BootstrapContext::runtimeState()?->customConfig() ?? [];
        $classesData    = $customConfig['character_class']                  ?? null;
        $rankingsFilter = $customConfig['rankings_classgroup_filter']       ?? null;
        ...
        foreach ($rankingsFilter as $class => $phrase) {
            if (! array_key_exists($class, $classesData)) {
                continue;
            }
            $translatedName = Translator::phrase((string) $phrase);
            $filterName     = $translatedName === 'ERROR' ? (string) $phrase : $translatedName;
            $classGroupList = [];
    ...
    private function rankingMenuLinks(): ?array
    {
