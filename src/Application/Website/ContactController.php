        ...
        try {
            ...
            if (isset($_POST['submit'])) {
                try {
                    ...
                    if (! Validator::Email($emailVal)) {
                        throw new \Exception(Translator::phrase('error_9'));
                    }
                    if (! Validator::Length($msgVal, 300, 10)) {
                        throw new \Exception(Translator::phrase('error_57'));
                    }
                    ...
                    $mail           = new Email();
                    $mail->_subject = BootstrapContext::moduleValue('subject');
                    $mail->setFrom($emailConfigs['send_from'], $emailConfigs['send_name'] . ' - Contact Form');
                    $mail->setReplyTo($emailVal);
                    $mail->_message = $msgVal;
                    $mail->addAddress(BootstrapContext::moduleValue('sendto'));
                    $mail->send();

                    MessageRenderer::toast('success', Translator::phrase('success_22'));
                } catch (\Exception $ex) {
