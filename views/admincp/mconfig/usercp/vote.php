...
$voteConfigUrl         = (string) ($voteConfigUrl ?? '');
$voteDeleteSiteUrlBase = (string) ($voteDeleteSiteUrlBase ?? '');
?>
<form action="<?php echo htmlspecialchars($voteConfigUrl, ENT_QUOTES, 'UTF-8'); ?>" method="post">
	<table class="table table-striped table-bordered table-hover module_config_tables">
		...
		<tr>
			...
			<td>
				<?php \Darkheim\Application\Shared\UI\FormFieldRenderer::enabledisableCheckboxes('setting_2', \Darkheim\Infrastructure\Bootstrap\BootstrapContext::moduleValue('vote_save_logs'), 'Enabled', 'Disabled'); ?>
			</td>
		</tr>
		<tr>
...
if (is_array($votesiteList)) {
    ...
    foreach ($votesiteList as $thisVoteSite) {
        ...
        echo '<td>' . $thisVoteSite['votesite_time'] . ' hour(s)</td>';
            echo '<td><a href="' . htmlspecialchars($voteDeleteSiteUrlBase . (string) $thisVoteSite['votesite_id'], ENT_QUOTES, 'UTF-8') . '" class="btn btn-block"><i class="fa fa-remove"></i></a></td>';
        echo '</tr>';
    }
} else {
...
echo '<form action="' . htmlspecialchars($voteConfigUrl, ENT_QUOTES, 'UTF-8') . '" method="post">';
echo '<tr>';
